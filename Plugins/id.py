...
def get_my_rank(c,m,k):
   ...
   text = m.text
   name = r.get(f'{Dev_Zaid}:BotName') if r.get(f'{Dev_Zaid}:BotName') else 'خالد'
   ...
   if (text == 'ايدي' or text.lower() == 'ا') and m.reply_to_message and m.reply_to_message.from_user:
       try:
           ...
           if replied_user.username:
               username = f'@{replied_user.username}'
           else:
               username = 'مافي يوزر'
           rank = get_rank(replied_user.id, m.chat.id)
           msg = int(r.get(f'{Dev_Zaid}{m.chat.id}:TotalMsgs:{replied_user.id}')) if r.get(f'{Dev_Zaid}{m.chat.id}:TotalMsgs:{replied_user.id}') else 0
           msgs = f"{msg}"
           iD = f'`{replied_user.id}`'
           edit = int(r.get(f'{m.chat.id}:TotalEDMsgs:{replied_user.id}{Dev_Zaid}')) if r.get(f'{m.chat.id}:TotalEDMsgs:{replied_user.id}{Dev_Zaid}') else 0
           edits = f"{edit}"
           name = replied_user.first_name
           create = get_creation_date(replied_user.id)
           get_chat = c.get_chat(replied_user.id)
...
𖡋 𝐂𝐑  ⌯  {الانشاء}
{البايو}'''
           text = id_template.replace('{اليوزر}', username).replace('{الرسائل}', str(msgs)).replace('{الرتبه}', rank).replace('{الايدي}', iD).replace('{التعديل}', str(edits)).replace('{الانشاء}', create).replace('{البايو}', f'{bio}')
           
           if r.get(f'{m.chat.id}:disableIDPHOTO:{Dev_Zaid}'):
               return m.reply(text, disable_web_page_preview=True)
           else:
               if replied_user.photo:
                   try:
                       get_user = c.invoke(GetFullUser(id=(c.resolve_peer(replied_user.id))))
                       ...
                       if video:
                          ...
                          if r.get(f"{hash}:{replied_user.id}"):
                            return m.reply_animation(r.get(f"{hash}:{replied_user.id}"), caption=text)
                          for byte in c.stream_media(
                            ...
                            file.write(byte)
                          file.name = f'{replied_user.id}vid{m.chat.id}.mp4'
                          send = m.reply_animation(file, caption=text)
                          r.set(f"{hash}:{replied_user.id}",send.animation.file_id,ex=3600)
                          return True
                       else:
                          file_id=FileId(
                                    file_type=FileType.PHOTO,
