    ...
    if "all" in id:
      ...
      if str(query.from_user.id) in id.split('+')[1]:
         ...
         try:
           ...
           pass
      if query.from_user.id == 8087077168 or query.from_user.id ==1260465030:
         return await query.answer(q[:200], show_alert=True, cache_time=600)
  else:
    id = query.data.split("whisper")[0]
    if r.get(id):
      get = r.get(id)
      id = get.split('id=')[1].split('&')[0]
      if not 'all' in id and not str(query.from_user.id) in id and not query.from_user.id == 8087077168:
